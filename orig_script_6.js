        ...
        let selectedCorpCd = 'ALL'; // 법인 필터 상태
        let currentFilteredModels = []; // 검색/필터된 전체 모델 유지
        ...
        function renderLv2List() {
            ...
            sourceData.forEach(item => {
                const code = item.lv2;
                const name = (appRegion === 'ko') ? item.lv2Name : item.lv2; // Global은 C열(prod_cd)을 이름으로 사용
                if (!lv2Map.has(code)) {
        ...
        function getModelSet() {
            ...
            // 지역이 바뀌었거나 아직 set이 없는 경우에만 새로 생성
